...
/*     $('a,button').mouseenter(function(){
    ...
    $('#photo').change(function(){
        let filePath = $(this).val().split('\\') /* split : 파일 경로까지 나오게 하는 거  */
        // console.log(filePath[2]);  /* >> 3번째 값만 써지게 하는 거 */ 
        let fileLength = filePath.length
        $('.file_name').text(filePath[fileLength-1]); 
        /* 만약 마지막 거를 구하려면
            8개면 7번째/ 4개면 3번째 그래서 -1한거
        */
    ...
    $('#secret').click(function(){
        ...
        $('.public_pw').css({backgroundColor: '#fff'})
        $('.public_pw').focus()
    })
    ...
    $('.btn_question').click(function() {
        let checkVal = $('.terms').is(':checked');  // 다른 예로 :checked 말고 :visible, :hidden도 있음
        // is() 는 true, false 값을 반환
    
        if (checkVal) {
            alert('약관에 동의하셨습니다.'); // 약관에 동의했을 때 메시지
        } else {
            alert('약관에 동의해 주세요.'); // 약관에 동의하지 않았을 때 메시지
        }
    });
}) 